...
pid_file="$service_dir/hybrid-evaluation.pid"
log_file="$service_dir/hybrid-evaluation.log"

mkdir -p "$service_dir"

if [ -f "$pid_file" ] && kill -0 "$(cat "$pid_file")" 2>/dev/null; then
  echo "混合检索评测已运行，PID=$(cat "$pid_file")"
  exit 0
fi
...
(
  ...
  nohup env \
    ...
    sh ./run-evaluation.sh hybrid \
      >"$log_file" 2>&1 </dev/null &
  echo "$!" >"$pid_file"
)

echo "300 题混合检索评测已启动，PID=$(cat "$pid_file")，日志=$log_file"
